...
def patient_data_form_fast(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and model:
            # Control Z: Mostrar datos limpios
            ...
            # Guardar datos en la sesión con conversión explícita a tipos nativos de Python
            request.session['AGE'] = int(patient_data.AGE)  # Convertir a entero nativo de Python
            request.session['GENDER'] = int(patient_data.GENDER)  # Convertir a entero nativo de Python
            request.session['prediction_result'] = int(prediction_result)  # Si es 0 o 1
            request.session['prediction_proba'] = float(prediction_proba * 100)  # Convertir a flotante

            # Guardar factores de riesgo positivos (convertidos a cadenas)
            factors = [
                ...
                ("CHRONIC_DISEASE", patient_data.CHRONIC_DISEASE),
                ("FATIGUE", patient_data.FATIGUE),
                ("ALLERGY", patient_data.ALLERGY),
                ("WHEEZING", patient_data.WHEEZING),
                ("ALCOHOL_CONSUMING", patient_data.ALCOHOL_CONSUMING),
                ("COUGHING", patient_data.COUGHING),
                ("SHORTNESS_OF_BREATH", patient_data.SHORTNESS_OF_BREATH),
                ("SWALLOWING_DIFFICULTY", patient_data.SWALLOWING_DIFFICULTY),
                ("CHEST_PAIN", patient_data.CHEST_PAIN),
            ]
            positive_factors = [key for key, value in factors if value == 1]
            request.session['positive_factors'] = positive_factors  # Guardar lista de cadenas
